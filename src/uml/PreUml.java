 ...
 * 
 */
public class PreUml extends JFrame implements ActionListener {
		...
		if (e.getSource() == button) {
			...
			if (! sta.matches("[0-9]*")) {
				...
				if(st.length()!=0 && sta.length()!=0) {
					...
					GestionFichier sysFichiers = new GestionFichier(PreUml.getJTextField1());
					...
					SocketServeurTCP serveurTCP = new SocketServeurTCP(protocole,Integer.parseInt(PreUml.getJTextField2()));
					serveurTCP.start();

					Uml uml = new Uml(sysFichiers);
				}else {
