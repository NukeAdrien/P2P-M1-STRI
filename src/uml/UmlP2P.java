...
/*
 * Classe UmlP2P : Génère la fenêtre JFrame destinée à au téléchargement P2P.
 * 
 */
public class UmlP2P extends JFrame implements ActionListener {
	/*
	...
	/* Déclaration et initialisation des JLabel/JTextArea */

	JLabel j = new JLabel("Entrez le nom du fichier à télécharger : ",SwingConstants.CENTER);
	JTextArea jta = new JTextArea();
	JLabel j2 = new JLabel("Entrez l'adresse IP de l'annuaire : ", SwingConstants.CENTER);
	JTextArea jta1 = new JTextArea();
	JLabel j3 = new JLabel("Entrez le numéro de port : ", SwingConstants.CENTER);
	JTextArea jta2 = new JTextArea();


	/* Création d'un bouton nécessaire pour le téléchargement */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (! jta1.getText().matches("^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$")) {
			...
			if (jta.getText().length()!=0 && jta1.getText().length()!=0 && nb!=0) {
				...
				controle.TelechargementSimple(jta.getText(), jta1.getText(), nb );
			}else {
